...
class DiscordHandler(metaclass=SingletonMeta):
    ...
    async def init_client(self) -> discord.Client:
        ...
        async def start_discord_client(discord_client: discord.Client, token: str):
            try:
                ...
                logger.info(
                    "Discord client started the session: %s, with identity: %s",
                    config.application.name,
                    discord_client.user.id,
                )  # type: ignore

            except (discord.LoginFailure, TypeError) as login_failure:
                logger.error("Error while connecting to Discord: %s", login_failure)
                sys.exit(1)
            except discord.HTTPException as http_exception:
                logger.critical(
                    "Discord client failed to connect with status: %s - %s",
                    http_exception.status,
                    http_exception.response.reason,
                )
    ...
    async def forward_message(
        ...
        try:
            ...
            logger.error(
                "Error while sending message to Discord: %s",
                http_exception,
                exc_info=Config.get_instance().application.debug,
            )
    ...
    async def fetch_reference(
        self, message: TelegramMessage, forwarder_name: str, discord_channel
        ...
        logger.debug(
            "Fetching reference Discord message for TG message %s", reply_to_msg_id
        )

        discord_message_id = await self.history_manager.get_discord_message_id(
            forwarder_name, reply_to_msg_id
        )
        if not discord_message_id:
            logger.debug("No mapping found for TG message %s", reply_to_msg_id)
            return None

        try:
            messages = []
            async for message in discord_channel.history(
                around=discord.Object(
            ...
            logger.debug(
                "Reference Discord message not found for TG message %s", reply_to_msg_id
            )
            return None

    def get_mention_roles(
        self,
    ...
    @staticmethod
    def is_builtin_mention_role(
        role_name: str, discord_built_in_roles: List[str]
    ) -> bool:
